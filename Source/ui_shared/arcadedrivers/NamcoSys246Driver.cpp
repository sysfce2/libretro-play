...
void CNamcoSys246Driver::PrepareEnvironment(CPS2VM* virtualMachine, const ARCADE_MACHINE_DEF& def)
{
	...
	//Override mc0 device with special device reading directly from zip file
	{
		...
		auto cdvdManModule = iopBios->GetCdvdman();
		if(def.ilinkId.has_value())
		{
			cdvdManModule->SetIlinkId(def.ilinkId.value());
		}
		...
		auto acCdvdModule = std::make_shared<Iop::Namco::CAcCdvd>(*iopBios->GetSifman(), *cdvdManModule, virtualMachine->m_iop->m_ram, *acRam.get());
